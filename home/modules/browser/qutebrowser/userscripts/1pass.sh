...
cat <<EOF
    ...
    var selectedForm = document.getElementsByTagName("form");
...
if [[ -z "$OP_ACCOUNT" ]]; then
    echo "message-error '1Password account not configured. Run: op account add'" >> "$QUTE_FIFO"
    exit 1
fi
...
if ! LIST_ITEM_OUT="$(op item list --cache --format=json)"; then
...
ITEM="$(op item get --cache --format=json "$UUID")"
...
TOTP="$(echo "$ITEM" | op item get --cache --otp "$UUID")" || TOTP=""
if [ -n "$TOTP" ]; then
